...
type Lexer struct {
	input string
	position int // starting position for the lex rule
	readPosition int // points to the next positon after position index
	ch byte //currnt character under examination
}
...
func (l *Lexer) NextToken() token.Token {
	...
	switch l.ch {
		...
		case '*': tok = newToken(token.ASTERIK, l.ch)
		case '!': 
			if l.peekChar() == '=' {
				ch := l.ch
				l.readChar()
